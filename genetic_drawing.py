...
class GeneticDrawing:
    def __init__(self, img_path, seed=0, brushesRange=[[0.1, 0.3], [0.3, 0.7]]):
        ...
        self.img_grey = prep(self.original_img)
    ...
    def set_sampling_mask(self, img):
        try:
            self.sampling_mask = cv2.cvtColor(cv2.imread(img),cv2.COLOR_BGR2GRAY)
        except:
            self.sampling_mask = img
    ...
    def pre_load_buffer(self, path, image=None):
        buf = []

        if not image:
            for img in os.listdir(path):
                buf.append(prep(cv2.imread(os.path.join(path, img))))
        else:
            buf.append(prep(cv2.imread(os.path.join(path, image))))
